...
def parse_args():
    ...
    parser.add_argument(
        "--save_outputs_dir", type=str, default='data/heatmap',
        help="Debug option. If specified, will save model predictions and ground truths to this directory. "
             "Specifically, will save `{pred_frames,pred_logits,gtruth_frames,gtruth_tokens}.pt` and error heatmaps."
    )
    parser.add_argument(
...
def main():
    ...
    # val_dataset.data = val_dataset.data[:64]
    ...
    if args.save_outputs_dir is not None:
        ...
        # Create directory for error heatmaps
        error_heatmaps_dir = Path(args.save_outputs_dir) / "error_heatmaps"
        os.makedirs(error_heatmaps_dir, exist_ok=True)
        heatmap_counter = 0  # To uniquely name each heatmap image
    ...
    for batch in tqdm(dataloader):
        ...
        # Convert samples to images using decode_latents_wrapper
        decoded_samples = evaluator.predict_next_frames(samples)  # Shape: (B, T-1, 3, 256, 256)

        ## Compute error heatmaps and save
        if args.save_outputs_dir is not None:
            # Decode ground truth frames
            decoded_gtruth = evaluator.predict_next_frames(reshaped_input_ids[:, 1:])  # Shape: (B, T-1, 3, 256, 256)

            # Loop through each sample in the batch
            for i in range(batch_size):
                episode_frames = []
                for t in range(WINDOW_SIZE - 1):
                    # Ground truth and predicted frames
                    gt_frame = decoded_gtruth[i, t].cpu().numpy().transpose(1, 2, 0)  # (256, 256, 3)
                    pred_frame = decoded_samples[i, t].cpu().numpy().transpose(1, 2, 0)  # (256, 256, 3)

                    # Ensure pixel values are in [0, 1]
                    gt_frame = np.clip(gt_frame, 0, 1)
                    pred_frame = np.clip(pred_frame, 0, 1)

                    # Compute absolute error per pixel
                    error = np.abs(gt_frame - pred_frame)  # (256, 256, 3)
                    # Convert to grayscale by averaging the error across color channels
                    error_gray = np.mean(error, axis=2)  # (256, 256)

                    # Normalize error for heatmap
                    error_normalized = (error_gray - error_gray.min()) / (error_gray.max() - error_gray.min() + 1e-8)

                    # Create heatmap
                    plt.figure(figsize=(6, 6))
                    # plt.imshow(error_normalized, cmap='hot', interpolation='nearest')
                    plt.axis('off')
                    plt.tight_layout()

                    # Save heatmap to a temporary buffer
                    heatmap_path = error_heatmaps_dir / f"heatmap_temp_{heatmap_counter}.png"
                    plt.savefig(heatmap_path, bbox_inches='tight', pad_inches=0)
                    plt.close()

                    # Read the saved heatmap and convert to array
                    heatmap_image = plt.imread(heatmap_path)
                    # Resize heatmap to match frame size if necessary
                    if heatmap_image.shape[:2] != gt_frame.shape[:2]:
                        heatmap_image = np.array(Image.fromarray((heatmap_image * 255).astype(np.uint8)).resize((256, 256)))
                    # Remove temporary heatmap file
                    os.remove(heatmap_path)

                    # Stack GT, Prediction, and Heatmap horizontally
                    combined_frame = np.hstack((gt_frame, pred_frame, heatmap_image))
                    episode_frames.append(combined_frame)
                    heatmap_counter += 1

                # Stack all frames vertically to form the episode image
                episode_image = np.vstack(episode_frames)

                # Convert to uint8 for saving
                episode_image_uint8 = (episode_image * 255).astype(np.uint8)

                # Save the episode image
                episode_path = error_heatmaps_dir / f"error_heatmap_episode_{heatmap_counter}.png"
                plt.imsave(episode_path, episode_image_uint8)
    ...
    if args.save_outputs_dir is not None:
        ...
        print(f"Error heatmaps saved to {error_heatmaps_dir}")
