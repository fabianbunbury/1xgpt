...
class RawTokenDataset(TorchDataset):
    ...
    def __init__(
        ...
        video_tokens_path, segment_ids_path, action_tokens_path = [data_dir / f"{name}.bin"
                                                                   for name in ["video", "segment_ids", "actions/driving_command"]]
...
def get_maskgit_collator_obs_actions(config: GenieConfig):
    mask_token_id = config.image_vocab_size
    h = w = math.isqrt(config.S)

    def collate_fn_obs_actions(features) -> dict[str, torch.Tensor]:
        # during training, map (z_0, z_1', z_2') -> (null, z_1, z_2)
        # (z_0, z_1') -> (null, z_1) is the diffusion operator on z_1' -> z_1

        input_ids = torch.stack([ex["input_ids"] for ex in features])
        joint_pos = torch.stack([ex["joint_pos"] for ex in features])
        l_hand_closure = torch.stack([ex["l_hand_closure"] for ex in features])
        driving_command = torch.stack([ex["driving_command"] for ex in features])
        neck_desired = torch.stack([ex["neck_desired"] for ex in features])
        r_hand_closure = torch.stack([ex["r_hand_closure"] for ex in features])
    ...
    return collate_fn_obs_actions


class RawTokenDatasetObsAction(RawTokenDataset):
    """ Loads raw uint32 tokens as memmap-backed array """
    def __init__(
        self,
        data_dir,
        ...
        shape = (self.metadata["num_images"], self.metadata["s"], self.metadata["s"])
        video_tokens_path, segment_ids_path, driving_command, joint_pos, l_hand_closure, neck_desired, r_hand_closure, = [data_dir / f"{name}.bin"
                                                                   for name in ["video", "segment_ids", "actions/driving_command", "actions/joint_pos", "actions/l_hand_closure", "actions/neck_desired", "actions/r_hand_closure"]]
        token_dtype = np.dtype(self.metadata.get("token_dtype", "uint32"))
        self.data = np.memmap(video_tokens_path, dtype=token_dtype, mode="r", shape=shape)
        
        self.driving_command = np.memmap(driving_command, dtype=np.uint16, mode="r", shape=(self.metadata["num_images"],))
        self.joint_pos = np.memmap(joint_pos, dtype=np.uint16, mode="r", shape=(self.metadata["num_images"],))
        self.l_hand_closure = np.memmap(l_hand_closure, dtype=np.uint16, mode="r", shape=(self.metadata["num_images"],))
        self.neck_desired = np.memmap(neck_desired, dtype=np.uint16, mode="r", shape=(self.metadata["num_images"],))
        self.r_hand_closure = np.memmap(r_hand_closure, dtype=np.uint16, mode="r", shape=(self.metadata["num_images"],))
    ...
    def __getitem__(self, idx):
        ...
        driving_command = torch.from_numpy((self.driving_command[start_ind : start_ind + self.video_len + 1 : self.stride]).astype(np.int16)).repeat_interleave(256)
        joint_pos = torch.from_numpy((self.joint_pos[start_ind : start_ind + self.video_len + 1 : self.stride]).astype(np.int16)).repeat_interleave(256)
        l_hand_closure = torch.from_numpy((self.l_hand_closure[start_ind : start_ind + self.video_len + 1 : self.stride]).astype(np.int16)).repeat_interleave(256)
        neck_desired = torch.from_numpy((self.neck_desired[start_ind : start_ind + self.video_len + 1 : self.stride]).astype(np.int16)).repeat_interleave(256)
        r_hand_closure = torch.from_numpy((self.r_hand_closure[start_ind : start_ind + self.video_len + 1 : self.stride]).astype(np.int16)).repeat_interleave(256)
